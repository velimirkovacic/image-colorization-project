...
class CocoDatasetTensor(Dataset):
    def __init__(self, paths):
        self.paths = paths
...
def get_loaders(path, batch_size=16, num_workers=4, n=8000):
...
def get_loaders_optimized(path_train, path_test, batch_size=64, num_workers=8):
    train_paths = glob.glob(path_train + "/*.pt")
    test_paths = glob.glob(path_test + "/*.jpg")

    train_dataset = CocoDatasetTensor(train_paths)
    test_dataset = CocoDataset(test_paths)
...
def show_batch(dataloader):
    ...
    num_imgs_to_plot = min(4, color_imgs.size(0) // 4)
