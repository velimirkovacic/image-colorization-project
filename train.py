...
class LitModel():
    ...
    def plot_losses(self):
        iterations = len(self.accumulator.data) // 2
        generator = [self.accumulator.data[i] for i in range(0, iterations * 2, 2)]
        discriminator = [self.accumulator.data[i] for i in range(1, iterations * 2, 2)]
        plt.plot([i for i in range(1, iterations + 1)], generator, label='Generator Loss')
        plt.plot([i for i in range(1, iterations + 1)], discriminator, label='Discriminator Loss')
